...
const TeamBuilder: React.FC<TeamBuilderProps> = ({ players }) => {
  ...
  if (players.length < 4) return <div className="p-8 text-center text-gray-500"><Users className="w-12 h-12 mx-auto mb-4 opacity-50" /><p>Need at least 4 confirmed players.</p></div>;
    ...
    <div className="space-y-6 pb-20">
      <div className="bg-gradient-to-br from-indigo-900/50 to-purple-900/50 rounded-2xl p-6 border border-indigo-500/20 shadow-lg">
        <div className="flex items-center justify-between mb-4">
          <div><h2 className="text-xl font-bold text-white flex items-center gap-2"><Sparkles className="w-5 h-5 text-yellow-400" /> AI Coach</h2></div>
          <button onClick={handleGenerate} disabled={loading} className="bg-white text-indigo-900 px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 hover:bg-indigo-50 disabled:opacity-50">{loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />} {teams ? 'Regenerate' : 'Make Teams'}</button>
        </div>
        {error && <div className="bg-red-500/10 text-red-400 p-3 rounded-lg text-sm mb-4 border border-red-500/20">{error}</div>}
        {!teams && !loading && <div className="text-center py-8 text-indigo-300/60"><Shield className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>Tap "Make Teams" to balance.</p></div>}
